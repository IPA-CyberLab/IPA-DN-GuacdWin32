 ...
 * @return 0 on success, otherwise a Win32 error code
 */
UINT rail_send_pdu_server(RailServerContext* context, wStream* s, UINT16 orderType)
{
	UINT16 orderLength;
                                       ...
                                       const RAIL_HANDSHAKE_ORDER* handshake)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_HANDSHAKE);
	Stream_Free(s, TRUE);
	return error;
}
                                          ...
                                          const RAIL_HANDSHAKE_EX_ORDER* handshakeEx)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_HANDSHAKE_EX);
	Stream_Free(s, TRUE);
	return error;
}
                                      ...
                                      const RAIL_SYSPARAM_ORDER* sysparam)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_SYSPARAM);
	Stream_Free(s, TRUE);
	return error;
}
                                             ...
                                             const RAIL_LOCALMOVESIZE_ORDER* localMoveSize)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_LOCALMOVESIZE);
	Stream_Free(s, TRUE);
	return error;
}
                                          ...
                                          const RAIL_MINMAXINFO_ORDER* minMaxInfo)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_MINMAXINFO);
	Stream_Free(s, TRUE);
	return error;
}
                                          ...
                                          const RAIL_TASKBAR_INFO_ORDER* taskbarInfo)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_TASKBARINFO);
	Stream_Free(s, TRUE);
	return error;
}
                                          ...
                                          const RAIL_LANGBAR_INFO_ORDER* langbarInfo)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_LANGBARINFO);
	Stream_Free(s, TRUE);
	return error;
}
                                         ...
                                         const RAIL_EXEC_RESULT_ORDER* execResult)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_EXEC_RESULT);
	Stream_Free(s, TRUE);
	return error;
}
                                          ...
                                          const RAIL_ZORDER_SYNC* zOrderSync)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_ZORDER_SYNC);
	Stream_Free(s, TRUE);
	return error;
}
...
static UINT rail_send_server_cloak(RailServerContext* context, const RAIL_CLOAK* cloak)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_CLOAK);
	Stream_Free(s, TRUE);
	return error;
}
                                       ...
                                       const RAIL_POWER_DISPLAY_REQUEST* powerDisplayRequest)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_POWER_DISPLAY_REQUEST);
	Stream_Free(s, TRUE);
	return error;
}
                                             ...
                                             const RAIL_GET_APPID_RESP_ORDER* getAppidResp)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_GET_APPID_RESP);
	Stream_Free(s, TRUE);
	return error;
}
                                               ...
                                               const RAIL_GET_APPID_RESP_EX* getAppidRespEx)
{
	...
	error = rail_send_pdu_server(context, s, TS_RAIL_ORDER_GET_APPID_RESP_EX);
	Stream_Free(s, TRUE);
	return error;
}
